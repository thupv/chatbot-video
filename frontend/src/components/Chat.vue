      ...
      >
        ...
        >
          <p>
            ...
            >
            <time class="message__meta" datetime="message.createdAt">{{message.createdAt | formatDatetime}}
            </time>
          </p>
          <p v-for="msg in message.data.messages" :key="msg">{{ msg }}</p>
          <div v-for="btn in message.data.buttons" :key="btn.state" class="message-btn" v-on:click="clickButton(btn.state)">{{btn.display}}</div>
        </div>
      </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      clickButton(state) {
        this.socket.emit('sendMessage', state, error => {
          if (error) {
            throw new Error(error)
          }
          this.message = ''
