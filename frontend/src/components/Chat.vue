    ...
    <div class="chat__main">
      <div id="messages" class="chat__messages" v-chat-scroll="{ always: false, smooth: true }">
        ...
        >
          ...
          <div></div>
          <div v-for="msg in message.data.messages" :key="msg">
            <div
              v-if="typeof msg == 'object'"
              class="url-card"
              v-on:click="navigateToYoutube(msg.url)"
            >
              <img v-bind:src="msg.thumbnailUrl" alt="Thubnail" style="width:100%" />
              <div class="url-card-text-container">
                <p>{{msg.title}}</p>
              </div>
            </div>
            <p v-else>{{msg}}</p>
          ...
          <div
            v-for="btn in message.data.buttons"
            :key="btn.state"
            class="message-btn"
            v-on:click="clickButton(btn.state)"
          >{{btn.display}}</div>
        </div>
      </div>
